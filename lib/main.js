								...
								if (error) templateCallback("Unable to read template", templateInfo, error);
									...
									glob(filePathsPattern, function(error, filePaths) {
										if (error) taskCallback("Unable to get file paths", { pattern : filePathsPattern }, error);
											...
											var includesToProcess = filePaths.length,
											...
											filePaths.forEach(function(filePath) {
												...
												if (pathParts.length <= 1) includeCallback();
													...
													if (!match) includeCallback("File must be config or userdata", includeInfo);
													else {
															...
															case "configs":
																...
																if (pathParts.length <=3) {
																	...
																	if (!fileOrCommandMatch) {
																		includeCallback("Config file must be file or command", includeInfo);
																		ignore = true;
																	}
																	else {
														...
														if (!ignore) fs.stat(filePath, function(error, stat) {
															if (error) includeCallback("Unable to get file stats", includeInfo, error);
															...
															//need to support ignoring of windows system/hidden files here
																...
																if (error) includeCallback(error.errno === 34 ? "File not found" : "Unable to read file", includeInfo, error);
																else {
																	var includeObject, 
																		includeProperty,

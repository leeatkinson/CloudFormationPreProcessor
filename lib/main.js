...
function getConfigPaths(pathPatterns, workingDirectory, callback) {
	...
	pathPatterns.forEach(function(pattern) {
		console.log("Getting config matching " + pattern);
...
function getIncludes(config, templatePath, relativeDirectory, callback) {
	var includes = [];
	if (config) {
...
function processTemplate(config, regionNames, templatePath, compact, processedTemplatePath) {
	...
	jsonFile.readFile(templatePath, function (err, template) {
		if (err) console.error("Error reading '" + templatePath + "': " + err);
			...
			getIncludes(config, templatePath, templateDirectoryPath, function(err, includes) {
				if (err) console.error("Error getting includes: " + err);
				else {
					var tasksToProcess = (amiMappingsCount * regionNames.length) + includes.length;
						...
						//log("AMI search...", templatePath, { "Region": regionName, "Owner" : owner, "Name": name });
							getAmiId(regionName, owner, name, function(err, amiId) {
								if (err) console.error("Error getting AMI owned by '" + owner + "' and named" + name + " in region '" + regionName + "'");
									...
									taskComplete(--tasksToProcess, templatePath, processedTemplatePath, template, compact);
								}
							});
						});
					});
...
function processConfigs(regionName, configPathPatterns, workingDirectory, compact, processedExtension, callback) {
	getRegionNames(regionName, function(err, regionNames) {
		if (err) console.error("Error getting AWS regions: " + err);
			...
			getConfigPaths(workingDirectory, configPathPatterns, function (err, configPaths) {
				if (err) console.error("Error getting config paths '" + configPathPatterns + "': " + err);
				else {
					var configsToProcess = configPaths.length;
					configPaths.forEach(function(configPath) {
						...
						jsonFile.readFile(configPath, function(err, config) {
							if (err) {
								...
								processTemplate(config, regionNames, templatePath, compact, processedTemplatePath, function(err) {
									if (err) callback(err);
									else if (--configsToProcess === 0) {
										callback(null);
									}
								});
							}

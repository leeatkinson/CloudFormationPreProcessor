...
function getFileIncludes(config, relativeDirectory) {
	var fileIncludes = [];
	if (config) {
		var configFleIncludes = config.fileIncludes;
		if (configFleIncludes) {
			var resources = configFleIncludes.resources;
			if (resources) {
				var directory = configFleIncludes.directory;
				if (directory) {
...
function getCommandIncludes(config, relativeDirectory) {
	return [];
}
...
function processTemplate(config, regionNames, templatePath, compact, processedTemplatePath) {
	...
	jsonFile.readFile(templatePath, function (err, template) {
		...
		if (err) console.error("Error reading '" + templatePath + "': " + err);
			...
			var fileIncludes = getFileIncludes(config, templateDirectoryPath);
			var fileIncludesCount = fileIncludes.length;

			var commandIncludes = getCommandIncludes(config, templateDirectoryPath);
			var commandIncludesCount = commandIncludes.length;

			console.log("Template '" + templatePath + "': " 
				+ amiMappingsCount + " AMI mappings; " 
				+ fileIncludesCount + " file-includes; "
				+ commandIncludesCount + " command-includes; "
			);
			
			var tasksCount = (amiMappingsCount * regionNames.length) + fileIncludesCount + commandIncludesCount;

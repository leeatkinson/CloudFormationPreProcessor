    ...
    regionNames.forEach(function(regionName) {
        ...
        getRegionAmiId(templateName, mappingName, regionName, amiNamePattern, amiOwner, templateRegionMapping ? templateRegionMapping.ID : null, function(error, result){
            if (!error) {
                ...
                templateObject[regionName] = { ID: result.id };
            }
            if (--toDo === 0) {
...
function processIncludeFileKeyParts(keyParts) {
...
function parseIncludeText(includeInfo, includeTemplate, propertyName, text) {
    ...
    if (contentObject && (includeInfo.type === "userdata")) contentObject = { "Fn::Base64": contentObject };
...
function processInclude(templateName, resourcesTemplate, includePath, includePathParts, callback) {
    ...
    var resourceTemplate = resourcesTemplate[includePathParts[1]],
        configsOrUserdataMatch = includePathParts[2].match(/^configs|userdata(\.ps1|\.cmd|\.sh|)$/);
    ...
    loadInclude(includePath, function (error, text) {
        ...
        switch (includeType) {
            ...
            case "command":
                ...
                switch (includeType) {
                    case "file":
                        processIncludeFileKeyParts(keyParts);
                ...
                includeTemplate = resourceTemplate.Metadata["AWS::CloudFormation::Init"][configName][includeType + "s"][includeInfo.includeKey = keyParts.join("/")];
                break;
            case "userdata":
                if (text) {
        ...
        if (!includeTemplate) {
            event("Template's include object not found", includeInfo);
            callback();
            return;
        }

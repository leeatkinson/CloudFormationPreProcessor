							...
							jsonFile.readFile(templatePath, function (error, template) {
								if (error) templateCallback("Unable to read template", templateInfo, error);
									...
									var directoryPath = templatePath + ".d",
										...
										filePathsPattern = resourcesPath + "/**";
									glob(filePathsPattern, function(error, filePaths) {
										if (error) taskCallback("Unable to get file paths", { pattern : filePathsPattern }, error);
											...
											filePaths.forEach(function(filePath) {
												var includeInfo = { template: templateName, path: filePath },
													pathParts = fspath.relative(resourcesPath, filePath).split(fspath.sep);
												if (pathParts.length <= 1) fileCallback();
												else if (pathParts.some(function(pathPart) { return pathPart === "."; })) fileCallback("Ignoring include as it is a hidden file", includeInfo);
												else {
													...
													if (!match) fileCallback("File must be config or userdata", includeInfo);
													else {
															...
															case "configs":
																...
																if (pathParts.length <=3) {
																	...
																	if (!fileOrCommandMatch) {
																		fileCallback("Config file must be file or command", includeInfo);
																		ignore = true;
																	}
																	else {
														...
														if (!ignore) fs.stat(filePath, function(error, stat) {
															if (error) fileCallback("Unable to get file stats", includeInfo, error);
															...
															//need to support ignoring of windows system/hidden files here
																...
																if (error) fileCallback(error.errno === 34 ? "File not found" : "Unable to read file", includeInfo, error);
																else {
																	var includeObject, 
																		includeProperty,
																	...
																	if (!includeObject) fileCallback("Template object not found", includeInfo);
																	else {
																		...
																		fileCallback(contentChanged ? "Include changed" : "Include unchanged", includeInfo);
																	}
																}
															});
														});

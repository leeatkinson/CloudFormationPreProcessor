...
function preProcess(region, templatePathPatterns, callback) {
	getEC2Client(region).describeRegions(function(error, regionsDescription) {
		if (error) event("Unable to get regions", { region: region }, error, callback);
		else {
			var regions = regionsDescription.Regions.map(function(o) { return o.RegionName; });

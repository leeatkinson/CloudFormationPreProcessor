...
function getFileIncludes(config, relativeDirectory) {
	var fileIncludes = [];
	forEach(config.fileIncludes, function(fileInclude, resourceKey) {
...
function getAmiId(regionName, namePattern, callback) {
	getEC2Client(regionName).describeImages({ 
		...
		if (err) callback(err);
			...
			if (images.length == 0) callback(new Error("AMI matching '" + namePattern + "' not found"));
...
function processTemplate(config, regionNames, templatePath, compact, processedExtension) {
	console.log("Template path is " + templatePath);
	jsonFile.readFile(templatePath, function (err, template) {
		//console.log("Tenplate is " + JSON.stringify(template));
		if (err) console.error("Error reading '" + templatePath + "': " + err);
			...
			amiMappings.forEach(function(amiMappingTask) {
				...
				var amiName = amiMappingTask.amiName;
				console.log("Template '" + templatePath + "', mapping '" + mappingKey + "': searching for last AMI with name matching  '" + amiName + "'");
				var mapping = mappings[mappingKey];
				emptyObject(mapping);
				regionNames.forEach(function(regionName) {
					getAmiId(regionName, amiName, function(err, amiId) {
						if (err) console.error("Error getting AMI named '" + amiName + "' in region '" + regionName + "'");
						else {
			...
			fileIncludes.forEach(function(fileInclude) {
				...
				fs.readFile(includePath, "utf8", function (err, content) {
					if (err) {
						if (err.errno == 34) console.error("File-include " + includePath + " not found");
						else console.error("Error reading file-include " + includePath + ": " + err);
					}
					else {
						resources[resourceKey].Metadata["AWS::CloudFormation::Init"][configKey].files[fileKey].content = content;
						taskComplete(--tasksCount, templatePath, processedTemplatePath, template, compact);
					}
				});
			});
					...
					configPaths.forEach(function(configPath) {
						...
						jsonFile.readFile(configPath, function(err, config) {
							if (err) {
								...
								processTemplate(config, regionNames, templatePath, compact, processedExtension);
							}
						});
					});
				}
